...
-- Dune theme colors (generated by matugen into ~/.config/themes/current).
-- Parse the `$name = rgba(...)` lines so border/shadow colors follow the wallpaper.
-- Falls back to the previous hardcoded values if the file is missing.
local theme_colors = {}
do
    local f = io.open(os.getenv("HOME") .. "/.config/themes/current/colors-hyprland.conf", "r")
    if f then
        for line in f:lines() do
            local name, value = line:match("^%s*%$([%w_]+)%s*=%s*(rgba?%b())")
            if name and value then
                theme_colors[name] = value
            end
        end
        f:close()
    end
end
local function tc(name, fallback)
    return theme_colors[name] or fallback
end
...
hl.on("hyprland.start", function()
    local startup = {
        ...
        "hyprpaper",
        scripts_path .. "/restore-wallpaper.sh",
...
hl.config({
    ...
    general = {
        ...
        border_size = 2,
        col = {
            active_border = grad({ tc("activeBorder1", "rgba(ffffffaa)"), tc("activeBorder2", "rgba(5c5c5c99)") }, 45),
            inactive_border = tc("inactiveBorder", "rgba(595959aa)"),
        },
    ...
    group = {
        groupbar = {
            ...
            rounding_power = 2.5,
            col = {
                inactive = grad({ tc("groupInactive1", "rgba(ffffff29)"), tc("groupInactive2", "rgba(5c5c5c99)") }, 90),
                active = grad({ tc("groupActive1", "rgba(ffffffaa)"), tc("groupActive2", "rgba(5c5c5c99)") }, 45),
                locked_inactive = grad({ tc("lockedInactiveBorder", "rgba(a3948199)"), "rgba(5c5c5c00)" }, 90),
                locked_active = grad({ tc("lockedActiveBorder1", "rgba(f0e0ccb9)"), "rgba(ffffff00)" }, 90),
            },
        },
        col = {
            border_active = grad({ tc("activeBorder1", "rgba(ffffffaa)"), tc("activeBorder2", "rgba(5c5c5c99)") }, 45),
            border_inactive = tc("inactiveBorder", "rgba(595959aa)"),
            border_locked_active = grad({ tc("lockedActiveBorder1", "rgba(f0e0ccb9)"), tc("lockedActiveBorder2", "rgba(5c5c5c99)") }, 90),
            border_locked_inactive = grad({ tc("lockedInactiveBorder", "rgba(a3948199)"), tc("lockedInactiveBorder", "rgba(a3948199)") }, 90),
        },
    },
...
-- Sand-drift feel: overshoot softened to a gentle settle, longer easing tails.
hl.curve("overshot", { type = "bezier", points = { { 0.05, 0.9 }, { 0.15, 1.0 } } })
hl.curve("swipe", { type = "bezier", points = { { 0.05, 0.85 }, { 0.2, 1.0 } } })

hl.animation({ leaf = "fadeSwitch", enabled = true, speed = 3.4, bezier = "default" })
hl.animation({ leaf = "windowsIn", enabled = true, speed = 1.2, bezier = "overshot" })
hl.animation({ leaf = "windowsOut", enabled = true, speed = 1.2, bezier = "overshot" })
hl.animation({ leaf = "windowsMove", enabled = true, speed = 2.3, bezier = "overshot" })
hl.animation({ leaf = "workspaces", enabled = true, speed = 2.3, bezier = "swipe", style = "slide" })

-----------------
-- Window rules
-----------------

-- Brave PWAs use an app-id-shaped class such as
-- brave-<app-id>-Profile_4. Keep normal Brave windows unaffected.
hl.window_rule({
    name = "brave-pwa-fake-fullscreen",
    match = {
        class = "^brave-[a-z0-9]+-(Default|Profile_[0-9]+)$",
    },
