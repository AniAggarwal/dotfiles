...
require("diffview").setup({
    ...
    keymaps = {
      ...
      diff3 = {
        ...
        { { "n", "x" }, "<leader>go", require("diffview.actions").diffget("ours"), { desc = "Obtain the diff hunk from OURS" } },
        { { "n", "x" }, "<leader>gt", require("diffview.actions").diffget("theirs"), { desc = "Obtain the diff hunk from THEIRS" } },
        { "n", "g?", require("diffview.actions").help({ "view", "diff3" }), { desc = "Open the help panel" } },
      },
      diff4 = {
        -- Mappings in 4-way diff layouts
        { { "n", "x" }, "<leader>gb", require("diffview.actions").diffget("base"), { desc = "Obtain the diff hunk from BASE" } },
        { { "n", "x" }, "<leader>go", require("diffview.actions").diffget("ours"), { desc = "Obtain the diff hunk from OURS" } },
        { { "n", "x" }, "<leader>gt", require("diffview.actions").diffget("theirs"), { desc = "Obtain the diff hunk from THEIRS" } },
